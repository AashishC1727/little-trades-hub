...
serve(async (req)=>{
  ...
  try {
    ...
    if (req.method === 'GET' && path === '/p2p-listings') {
      ...
      // CORRECTED QUERY: The explicit foreign key hint has been removed.
      let query = supabase.from('p2p_listings').select(`
          *,
          profiles(display_name, avatar_url)
        `).eq('status', 'active').gt('expiry', new Date().toISOString());
      ...
      switch(sortBy){
        case 'ending_soon':
          ...
          break;
        case 'high_value':
          query = query.order('offered_value', {
            ascending: false,
            nullsLast: true
          });
          break;
    ...
    if (req.method === 'POST' && path === '/p2p-listings/create') {
      ...
      const { offered_asset, wanted_asset, category, description, asset_image_url, location, offered_value, wanted_value, expiry_days = 7 } = body;
    ...
    if (req.method === 'GET' && path === '/p2p-listings/my') {
      ...
      const { data, error } = await supabase.from('p2p_listings').select('*').eq('user_id', user.id).order('created_at', {
        ascending: false
      });
      if (error) throw error;
      return new Response(JSON.stringify({
    ...
    if (req.method === 'PATCH' && path.startsWith('/p2p-listings/')) {
      ...
      const body = await req.json();
      const { data, error } = await supabase.from('p2p_listings').update(body).eq('id', listingId).eq('user_id', user.id).select().single();
      if (error) throw error;
      return new Response(JSON.stringify({
        success: true,
        listing: data
    ...
    if (req.method === 'DELETE' && path.startsWith('/p2p-listings/')) {
      ...
      const { error } = await supabase.from('p2p_listings').delete().eq('id', listingId).eq('user_id', user.id);
      if (error) throw error;
      return new Response(JSON.stringify({
        success: true
      }), {
